...
#mask_b: iteration decoding initial mask, where mask_b is true means mask
    def inpainting(self,image,mask_b,i): #MakGIT inference
        ...
        self.model.eval()
        with torch.no_grad():
            z_indices = self.model.encode_to_z(image[0].unsqueeze(0)) #z_indices: masked tokens (b,16*16)
            ...
            for step in range(self.total_iter):
                ...
                print(f"Step {step}, mask sum: {mask_bc.sum().item()}, z_indices_predict sum: {z_indices_predict.sum().item()}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', type=str, default="cuda:3", help='Which device the training is on.')#cuda
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='checkpoints_c/best_val_loss_cosine.pt', help='load ckpt')
