...
class DQNAgent:
    ...
    def run(self, episodes=1000000):
        save_milestones = [200_000, 400_000, 600_000, 800_000, 1_000_000]
        for ep in range(episodes):
            ...
            while not done and step_count < self.max_episode_steps:
                ...
                if self.env_count % 1000 == 0:                 
                    ...
                    ########## END OF YOUR CODE ##########   
                if self.env_count in save_milestones:
                    model_path = os.path.join(self.save_dir, f"model_{self.env_count // 1000}k.pt")
                    torch.save(self.q_net.state_dict(), model_path)
                    print(f"[AutoSave] Saved model at step {self.env_count} → {model_path}")
    ...
    def train(self):
        ...
        batch = random.sample(self.memory, self.batch_size) # uniform sampling
