...
class Test_model(VAE_Model):
    ...
    def eval(self):
        ...
        print("val_loader 長度：", len(val_loader))  # 加在 for 迴圈前
        pred_seq_list = []
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        for i in range(1, 630):
            previous_frame = self.frame_transformation(decoded_frame_list[i-1].to(self.args.device))
            current_label = self.label_transformation(label[i]).to(self.args.device)
            z = torch.randn((1, self.args.N_dim, 32, 64), device=self.args.device) 
            decoded = self.Decoder_Fusion(previous_frame, current_label, z)
